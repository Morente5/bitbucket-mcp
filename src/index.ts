...
class BitbucketUrlBuilder {
   ...
   */
  repositoryBranchingModelSettings(
    workspace: string,
    repoSlug: string,
  ): string {
    if (this.type === BitbucketType.CLOUD) {
...
class BitbucketServer {
  ...
  private getServerPluginBaseUrl(pluginPath: string): string | null {
    ...
    const serverRoot = this.config.baseUrl.replace(
      /\/rest\/api\/[^/]+\/?$/,
      "",
    );
  ...
  constructor() {
    ...
    this.paginator = new BitbucketPaginator(
      this.api,
      logger,
      this.config.type === BitbucketType.SERVER ? "server" : "cloud",
    );
  ...
  async listRepositories(
    ...
    try {
      ...
      if (!wsName) {
        const paramName =
          this.config.type === BitbucketType.CLOUD
            ? "BITBUCKET_WORKSPACE"
  ...
  async getEffectiveDefaultReviewers(workspace: string, repo_slug: string) {
    try {
      ...
      if (this.config.type === BitbucketType.SERVER) {
        const pluginBase = this.getServerPluginBaseUrl(
  ...
  async getPullRequest(
    ...
    try {
      ...
      const response = await this.api.get(
        this.urlBuilder.pullRequest(
          workspace,
          repo_slug,
          parseInt(pull_request_id),
        ),
      );
  ...
  async approvePullRequest(
    ...
    try {
      ...
      const response = await this.api.post(
        this.urlBuilder.pullRequestApprove(
          workspace,
          repo_slug,
          parseInt(pull_request_id),
        ),
  ...
  async declinePullRequest(
    ...
    try {
      ...
      const response = await this.api.post(
        this.urlBuilder.pullRequestDecline(
          workspace,
          repo_slug,
          parseInt(pull_request_id),
        ),
  ...
  async mergePullRequest(
    ...
    try {
      ...
      const response = await this.api.post(
        this.urlBuilder.pullRequestMerge(
          workspace,
          repo_slug,
          parseInt(pull_request_id),
        ),
  ...
  async getPullRequestComments(
    ...
    try {
      ...
      // Bitbucket Server: the /comments endpoint requires a mandatory `path` query param,
      ...
      if (this.config.type === BitbucketType.SERVER) {
        const activitiesResult = await this.paginator.fetchValues<any>(
          ...
          ),
          {
            ...
            page,
            all: all ?? true,
            description: "getPullRequestComments (via activities)",
          },
        );

        // Extract comment objects from COMMENTED activities, enriching each with:
            ...
            return {
              ...
              ...(commentAnchor ? {
                commentAnchor: {
                  ...
                  lineType: commentAnchor.lineType,
                  fileType: commentAnchor.fileType,
                  orphaned,
                },
              } : {}),
  ...
  async getPullRequestDiff(
    ...
    try {
      ...
      if (this.config.type === BitbucketType.CLOUD) {
        ...
        diffUrl = this.urlBuilder.pullRequestDiff(
          workspace,
          repo_slug,
          parseInt(pull_request_id),
        );
  ...
  async getPullRequestCommits(
    ...
    try {
      ...
      const result = await this.paginator.fetchValues(
        this.urlBuilder.pullRequestCommits(
          workspace,
          repo_slug,
          parseInt(pull_request_id),
        ),
  ...
  async addPullRequestComment(
    ...
    try {
      ...
      if (inline) {
        if (this.config.type === BitbucketType.SERVER) {
          // Bitbucket Server uses "anchor" with line/lineType/fileType/diffType
          const isFromLine =
            inline.from !== undefined && inline.to === undefined;
          commentData.anchor = {
            line: inline.to ?? inline.from,
            lineType: isFromLine ? "REMOVED" : "ADDED",
  ...
  async getPendingReviewPRs(
    ...
    try {
      ...
      logger.info("Getting pending review PRs", {
        workspace: wsName,
        username:
          currentUserNickname ?? "(not set — filtering all REVIEWER PRs)",
      ...
      for (let i = 0; i < repositoriesToCheck.length; i += batchSize) {
        ...
        const batchPromises = batch.map(async (repoSlug) => {
          try {
            ...
            const values: any[] =
              prsResponse.data.values ?? prsResponse.data ?? [];
            if (!values.length) {
              return [];
            }
            ...
            const reposPendingPRs = values.filter((pr: any) => {
  ...
  async getPullRequestComment(
    ...
    try {
      ...
      const response = await this.api.get(
        this.urlBuilder.pullRequestComment(
          workspace,
          repo_slug,
          parseInt(pull_request_id),
          parseInt(comment_id),
  ...
  async getPullRequestDiffStat(
    ...
    try {
      ...
      const result = await this.paginator.fetchValues(
        this.urlBuilder.pullRequestDiffStat(
          workspace,
          repo_slug,
          parseInt(pull_request_id),
        ),
  ...
  async getPullRequestPatch(
    ...
    try {
      ...
      const patchUrl = this.urlBuilder.pullRequestPatch(
        workspace,
        repo_slug,
        parseInt(pull_request_id),
      );

      const response = await this.api.get(patchUrl, {
  ...
  async getPullRequestTasks(
    ...
    try {
      ...
      const result = await this.paginator.fetchValues(
        this.urlBuilder.pullRequestTasks(
          workspace,
          repo_slug,
          parseInt(pull_request_id),
        ),
  ...
  async getPullRequestStatuses(
    ...
    try {
      ...
      if (this.config.type === BitbucketType.SERVER) {
        const buildStatusBase = this.getServerPluginBaseUrl(
